...
- [Tasklist ledger](reference/tasklist-ledger.md) — every tasklist this repository has completed
  and the commit it merged as. Generated by `scripts/roadmap-ledger.sh --write`; the block between
  its markers is never hand-edited. A row here means a tasklist merged, which is a narrower claim
  than the capability behind it working — that one only the [roadmap](../ROADMAP.md) makes.
